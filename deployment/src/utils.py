...
def transform_images(pil_imgs: List[PILImage.Image], image_size: List[int], center_crop: bool = False) -> torch.Tensor:
    """Transforms a list of PIL images into a batched tensor for ViNT."""
    transform_type = transforms.Compose([
        transforms.Resize(image_size[::-1]),  # (H, W)
        transforms.ToTensor(),
    ...
    for pil_img in pil_imgs:
        ...
        transf_img = transform_type(pil_img)  # [3, H, W]
        transf_imgs.append(transf_img.unsqueeze(0))  # [1, 3, H, W]

    # ✅ batch dimension으로 합치기 (dim=0)
    return torch.cat(transf_imgs, dim=0)  # [N, 3, H, W]
